...
//this setup is in the loop of the stand alone 24 hour code
   setRegisterPin(white_LED, HIGH);//White with green led flash for 1 second indicates manual_mode
   setRegisterPin(ledPin, HIGH);//
   writeRegisters();

   delay(1000);

   setRegisterPin(white_LED, LOW);//White with green led flash for 1 second indicates manual_mode
...
//manual mode
 ...
 digitalWrite(setPin, LOW);           // Set HC-12 into AT Command mode
 delay(100);                          // Wait for the HC-12 to enter AT Command mode
 mySerial.print(F("AT+C001"));               // Send AT Command to HC-12 COO1 for chamber a1 and COO2 for chamber b1
 delay(100);
 digitalWrite(setPin, HIGH);
 delay(100);
...
while (counter<=8) {   //this is the time out counter for communication - to make sure that the valve changer has moved on to the next chamber and that the command recieved is the final command. 
...
{                 setRegisterPin(blue_LED, LOW); 
       ...
       pinMode(10, OUTPUT); //ss/ cs
       while (!SD.begin()) { //whilst the sd is not okay, the green and red led are turned on alternately //changed sd.begin (4) to sd begin () to use the automatic
        delay(2000);     //was delay(2000); but changed 29.03.2022 because thats too long
notOkayLED(200);
...
if (counter2>=5){  //i.e. if no command has been recieved for 5 minutes
